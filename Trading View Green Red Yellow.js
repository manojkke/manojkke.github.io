...
function calculatePercentage() {
  ...
  var smallGreenVal = greenVal;
  var smallRedVal = redVal;
  /////////////////////////////////chart 2 /////////////////////////////////////////////////
  ...
  $(".chart-toolbar").html(
    ' <button style="height: 100%;width: ' +
      redVal +
      'px;background: red;"> ' +
      redVal +
      '</button><button style="height: 100%;width: ' +
      greenVal +
      'px;background: green;">  ' +
      greenVal +
      "</button>"
  ...
  if (redVal <= 20 || greenVal <= 20) {
    var soundSrc =
      "https://www.myinstants.com/media/sounds/stupid-f__king-bird.mp3";
    if (chartSymbol.indexOf("ts") > -1) {
      soundSrc =
        "https://raw.githubusercontent.com/manojkke/manojkke.github.io/refs/heads/main/Tesla.mp3";
    } else if (chartSymbol.indexOf("nv") > -1) {
      soundSrc =
        "https://raw.githubusercontent.com/manojkke/manojkke.github.io/refs/heads/main/Nvdia.mp3";
    }

    $(".chart-toolbar").css({ backgroundColor: "yellow" });
    var audio = new Audio(soundSrc);
    audio.play();
  } else {
    $(".chart-toolbar").css({ backgroundColor: "" });
  }

  smallTimeFramChart(smallGreenVal, smallRedVal);
}

function smallTimeFramChart(greenVal, redVal) {
  redVal = redVal * 10;
  greenVal = greenVal * 10;

  if (redVal <= 20 || greenVal <= 20) {
    $(".chart-toolbar").append(
      '<span>______________________________________________________________________</span><button style="height: 100%;width: ' +
        redVal +
        'px;background: red;"> ' +
        redVal +
        '</button><button style="height: 100%;width: ' +
...
var chartSymbol =
  $($(".chart-container")[1]).text().indexOf("TSLA") > -1 ? "ts" : "nv";
